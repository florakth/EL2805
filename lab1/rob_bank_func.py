...
class Town:
    ...
    def __init__(self, maze, police_cant_stay=True):
        ...
        self.bank_pos = self._bank_position()
    ...
    def _bank_position(self):
        bankpositions = dict()
        bankpositions[1] = self.BANK1
        bankpositions[2] = self.BANK2
        bankpositions[3] = self.BANK3
        bankpositions[4] = self.BANK4
    ...
    def __minotaur_actions(self, state, cant_stay=True):
        ...
        if self.states[state][0] == self.states[state][2]:  # same line
            if self.states[state][1] > self.states[state][3]:  # police needs to go right, up, and down
                # print("C1")
                actionList = [2, 3, 4]
            else:  # police needs to go left, up, and down
                # print("C2")
                actionList = [1, 3, 4]

        if self.states[state][1] == self.states[state][3]:  # same column
            if self.states[state][0] > self.states[state][2]:  # police needs to go left, right, and down
                # print("C3")
                actionList = [1, 2, 4]
            else:  # police needs to go left, right, and up
                # print("C4")
                actionList = [1, 2, 3]

        if self.states[state][0] > self.states[state][2]:
            if self.states[state][1] > self.states[state][3]:  # police needs to go right, and down
                # print("C5")
                actionList = [2, 4]
            else:  # police needs to go left, and down
                # print("C6")
                actionList = [1, 4]

        if self.states[state][0] < self.states[state][2]:
            if self.states[state][1] > self.states[state][3]:  # police needs to go right, and up
                # print("C7")
                actionList = [2, 3]
            else:  # police needs to go left, and up
                # print("C8")
                actionList = [1, 3]
    ...
    def __rewards(self):
        ...
        for s in range(self.n_states):
            for a in range(self.n_actions):
                ...
                if [self.states[next_s][0], self.states[next_s][1]] == [self.states[next_s][2],
                                                                        self.states[next_s][3]]:
                    rewards[s, a] = self.CAUGHT_REWARD

                # Reward for reaching the bank
                elif (self.states[next_s][0], self.states[next_s][1]) in self.bank_pos.values() and not self.is_dead(next_s):
...
def SARSA(env, start, state, epsilon=0.1, steps=10000000):
    """ Solves the shortest path problem using dynamic programming
        :input Maze env           : The maze environment in which we seek to
                                    find the shortest path.
        :input int horizon        : The time T up to which we solve the problem.
